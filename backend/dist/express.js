...
app.get("/account/:id_acc/profile-image", async (req, res) => {
    ...
    const result = await pool.query(`SELECT image
         FROM account
         WHERE id_acc = $1`, [id_acc]);
    ...
    try {
     ...
     ORDER BY od.id_order_detail`, [id]);
        ...
        const order = {
            ...
            total: rows.reduce((sum, row) => sum + Number(row.subtotal), 0)
        };
...
app.post("/account/:id_acc/profile-image", upload.single("image"), async (req, res) => {
    const id_acc = Number(req.params.id_acc);
    if (Number.isNaN(id_acc)) {
        res.status(400).send("Invalid account id");
        return;
    ...
    const result = await pool.query(`UPDATE account
             SET image = $1
             WHERE id_acc = $2`, [req.file.buffer, id_acc]);
...
app.post("/order", async (req, res) => {
    ...
    try {
        ...
        const customerResult = await client.query(`SELECT id_customer
             FROM customers
             WHERE id_customer = $1`, [id_customer]);
        if (customerResult.rows.length === 0) {
            throw new Error("Customer not found");
        }
        const orderResult = await client.query(`INSERT INTO orders (id_customer)
             VALUES ($1)
        ...
        for (const item of items) {
            ...
            const productResult = await client.query(`SELECT id, name, price
                 FROM products
                 WHERE id = $1`, [item.id_product]);
            if (productResult.rows.length === 0) {
                throw new Error(`Product ${item.id_product} not found`);
            }
            const product = productResult.rows[0];
            await client.query(`INSERT INTO order_details
